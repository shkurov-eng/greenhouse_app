...
- Environment variables (see also `.env.example`):
  ...
  - `GEMINI_IMAGE_MODEL` (optional; overrides the default Gemini image model for room cartoon generation when cartoon uses the Gemini provider)
  - `OPENROUTER_API_KEY` (optional; when set, room cartoon generation uses OpenRouter `chat/completions` with image output modalities unless `CARTOON_IMAGE_PROVIDER=gemini`)
  - `OPENROUTER_CARTOON_MODEL` (optional; OpenRouter model id for cartoon, e.g. `google/gemini-2.5-flash-image`)
  - `CARTOON_IMAGE_PROVIDER` (optional; `openrouter` or `gemini`; defaults to OpenRouter if `OPENROUTER_API_KEY` is set, otherwise Gemini)
