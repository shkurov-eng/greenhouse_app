...
async function detectPlantProfileWithAiStudio(
  ...
  const { thirstyAfterHours: thirsty, overdueAfterHours: overdue } = normalizeWateringThresholds(
    thirstyRaw,
    overdueRaw,
  );
  ...
  return {
    profile: {
      plantName,
      thirstyAfterHours: thirsty,
      overdueAfterHours: overdue,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (aiProfile) {
      ...
      updatePayload.thirsty_after_hours = aiProfile.thirstyAfterHours;
      updatePayload.overdue_after_hours = aiProfile.overdueAfterHours;
