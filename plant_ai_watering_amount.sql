-- Store AI recommendation for watering amount per plant.

alter table if exists public.plants
  add column if not exists watering_amount_recommendation text;

alter table if exists public.plants
  drop constraint if exists plants_watering_amount_recommendation_check;

alter table if exists public.plants
  add constraint plants_watering_amount_recommendation_check
  check (
    watering_amount_recommendation is null
    or watering_amount_recommendation in ('light', 'moderate', 'abundant')
  );

-- Migrate legacy values if present.
update public.plants
set watering_amount_recommendation = 'light'
where watering_amount_recommendation = 'little';

update public.plants
set watering_amount_recommendation = 'abundant'
where watering_amount_recommendation = 'a_lot';
